...
namespace NDSRom
{
    public class Rom
    {
        ...
        public Rom(string path)
        {
            ...
            FileStream rom = new FileStream(path, FileMode.Open, FileAccess.Read);
        ...
        public byte[] GetFile(string romInternalFilePath)
        {
            ...
            rom.Position = this.fat.GetStartOffsetFromFileId(fileId);
        ...
        public void SaveRom(string newRomPath)
        {
            ...
            this.fileCopier.Copy(rom, 0x180, 0x3E80, newRom, 0x180);

            //Empieza con el codigo
            this.fileCopier.Copy(rom, this.header.ARM9RomOffset, this.header.ARM9CodeSize, newRom, this.header.ARM9RomOffset);
            this.fileCopier.Copy(rom, this.header.ARM9OverlayOffset, this.header.ARM9OverlaySize, newRom, this.header.ARM9OverlayOffset);
            this.fileCopier.Copy(rom, this.header.ARM7RomOffset, this.header.ARM7CodeSize, newRom, this.header.ARM7RomOffset);
            this.fileCopier.Copy(rom, this.header.ARM7OverlayOffset, this.header.ARM7OverlaySize, newRom, this.header.ARM7OverlayOffset);
            //Acaba con el codigo
            ...
            //Copia la FNT
            this.fileCopier.Copy(rom, this.header.FileNameTableOffset, this.header.FileNameTableSize, newRom, this.header.FileNameTableOffset);

            //Copia el banner
            this.fileCopier.Copy(rom, this.header.IconBannerOffset, 0x20 + 0x20 + 0x200 + 6 * 0x100, newRom, this.header.IconBannerOffset);
            ...
            for (ushort fileId = 0; fileId < this.fat.FilesAmount; fileId++)
            {
                ...
                {
                    this.fileCopier.Copy(rom, this.fat.GetStartOffsetFromFileId(fileId), this.fat.GetSizeFromFileId(fileId), newRom, this.modifiedRomFat.GetStartOffsetFromFileId(fileId));
                }
            }
